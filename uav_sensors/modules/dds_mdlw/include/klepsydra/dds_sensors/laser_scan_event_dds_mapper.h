...
{
public:
    void fromMiddleware(const kpsr_dds_sensors::LaserScanEvent &message,
                        kpsr::sensors::LaserScanEvent &event)
    {
        ...
        std::transform(message.ranges().begin(),
                       message.ranges().end(),
                       event.ranges.begin(),
                       [](const double eventItem) { return eventItem; });

        event.intensities.resize(message.intensities().size());
        std::transform(message.intensities().begin(),
                       message.intensities().end(),
                       event.intensities.begin(),
                       [](const double eventItem) { return eventItem; });
    }

    void toMiddleware(const kpsr::sensors::LaserScanEvent &event,
                      kpsr_dds_sensors::LaserScanEvent &message)
    {
        ...
        std::transform(event.ranges.begin(),
                       event.ranges.end(),
                       message.ranges().begin(),
                       [&](double eventData) { return eventData; });

        message.intensities().resize(event.intensities.size());
        std::transform(event.intensities.begin(),
                       event.intensities.end(),
                       message.intensities().begin(),
                       [&](double eventData) { return eventData; });
    }
};
} // namespace kpsr
